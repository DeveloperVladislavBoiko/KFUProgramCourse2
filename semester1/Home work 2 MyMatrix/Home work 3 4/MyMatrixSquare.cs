...
{
    public MyMatrixSquare(int n = 0) : base(n, n)
    {
    }
    ...
    {
        if (array == null)
            throw new ArgumentNullException($"матрица {nameof(array)} не может быть получена из аргумента null");
        
        if (array.GetLength(0) != array.GetLength(1))
            throw new Exception($"Матрица {array.GetLength(0)}x{array.GetLength(1)} не является квадратной");
    }

    public MyMatrixSquare(MyMatrixSquare other) : base(other)
    {
        if (other == null)
            throw new ArgumentNullException($"матрица {nameof(other)} не может быть создана из null");
    }

    public double Determinant()
    {
        if (N == 0) throw new Exception("Нельзя найти определитель для матрицы 0Х0");
    ...
    {
        if (N == 0) throw new Exception("Нельзя найти обратную матрицу для матрицы 0Х0");
